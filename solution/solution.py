...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            ...
            f"Answer: {answer}\n"
            f"Rubric:\n{rubric_lines}\n"
            "Score each rubric criterion from 0.0 to 1.0 and respond as JSON, "
            'e.g. {"accuracy": 0.8, "clarity": 0.7}.'
        )
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        return {
            "positional_bias": bool(first_scores) and avg_first > avg_all,
...
class FailureAnalyzer:
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("irrelevant"):
            suggestions.append(
                "Add few-shot examples and clarify prompt to keep answers on-topic"
            )
        ...
        if categories.get("off_topic"):
            suggestions.append(
                "Improve intent detection/routing to select the correct document set"
            )

        while len(suggestions) < 3:
            fallback = [
                "Add few-shot examples showing complete answers to improve completeness",
                "Expand golden dataset coverage to catch more failure patterns",
                "Review retrieval ranking to surface the most relevant chunks first",
            ]
            for s in fallback:
